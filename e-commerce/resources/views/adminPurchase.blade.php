        ...
        <div class="col-md-8">
            <div class="card bg-light shadow p-3 mb-5 bg-body rounded m-5 border-light">
                <div class="card-body">
                    ...
                    @if($history->isEmpty())
                        <h5 class="card-title text-center">No Transaction found</h5>
                    @else
                        @foreach ($history as $history)
                            <div class="purchase-item mb-4">
                                <div class="card mb-3">
                                    <div class="card-body">
                                        <h5 class="card-title">Transaction ID: {{ $history->id }}</h5>
                                        <p class="card-text text-muted">Username: {{ $history->user->username }}</p>
                                        <p class="card-text text-muted">Date: {{ $history->created_at }}</p>
                                        @foreach ($history->transaction as $product)
                                        ...
                                        <p class="card-text">Shipping fee: <small>Rp {{ number_format($history->ship_fee, 0, ',', '.') }}</small></p>
                                        <p class="card-text">Transaction Price: <small>Rp {{ number_format($history->total_price, 0, ',', '.') }}</small></p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
